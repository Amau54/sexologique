    ...
    panel.innerHTML = `
        <div class="deepclean-header" id="deepclean-header">
            <div class="deepclean-header-info">
                <h1>DeepClean V7.4 - Ultimate Edition</h1>
        ...
        <div class="deepclean-pane" data-pane="scanner">
            ...
            <ul class="scanner-list">
                ${state.scannerMode === 'audit' 
                    ...
                    : state.ghostClasses.filter(g => g.className.toLowerCase().includes(state.scannerSearch.toLowerCase()))
    ...
    return `
        <li class="scanner-rule-card ghost-scanner-card">
            ...
            <div class="scanner-actions">
                <button data-ghost-view='${ghost.className}'>Voir</button>
            </div>
        </li>`;
}
...
function renderAccordion(cls) {
    ...
    if (cls.isGhost || cls.hasVariant) {
        ...
        if (cls.isSaved) {
            content += `<div class="ghost-info" style="border-left: 3px solid var(--color-warning); padding-left: 8px;"><h3>Protection Durable</h3><p>Cette classe est écrasée par une règle <b>Responsive</b> (ex: une classe mobile ou tablette). Elle est conservée car elle est nécessaire sur d'autres tailles d'écran.</p></div>`;
        } else if (cls.isConditional && kept.length === 0) {
            content += `<div class="ghost-info" style="border-left: 3px solid var(--color-warning); padding-left: 8px;"><h3>Protection Conditionnelle</h3><p>Cette classe est inactive car elle ne s'applique qu'à un état spécifique (hover, focus) ou à une autre taille d'écran.</p></div>`;
        }
...
function addPanelEventListeners(analysisResults) {
    ...
    sr.getElementById('global-export-btn').addEventListener('click', async () => {
        ...
        for (let i = 0; i < total; i += batchSize) {
            ...
            showToast(`Nettoyage: ${i}/${total} éléments... (Batch ${Math.ceil((i+1)/batchSize)}/${Math.ceil(total/batchSize)})`);
        ...
        showToast('Nettoyage Terminé ! Téléchargement lancé.');
    });
    ...
    if (ghostModeBtn) {
        ghostModeBtn.addEventListener('click', async () => {
            ...
            showToast('Recherche des classes fantômes...');
    ...
    for (const className of classList) {
        ...
        for (const prop in declaredProps) { 
            if (originalComputedStyle[prop] !== computedStyleWithoutClass[prop]) {
                ...
                const winner = winningRulesMap[prop];
                if (winner && isRuleConditional(winner)) {
                    protectedByResponsive = true;
                }
            }
        }
        ...
        if (keptProps.length === 0) {
            // If it's trash here but protected by a conditional override, it's Partial
            category = (isConditional || protectedByResponsive) ? 'partial' : 'trash'; 
        ...
        results.push({ 
            ...
            isGhost: false, 
            isConditional,
            isSaved: protectedByResponsive && keptProps.length === 0, // Specifically for UI indicator
    ...
    function flatten(ruleList) {
        for (const rule of ruleList) {
            if (rule.type === CSSRule.STYLE_RULE) {
                ...
                // For Media, Supports, or Keyframes rules
                // We keep the grouping rule context in parentRule but flatten the actual style rules
...
function getWinningRulesMap(element, allRules) {
    ...
    // Sort by specificity and order (basic version)
    matchingRules.sort((a, b) => {
        const specA = calculateSpecificity(a.selectorText);
        const specB = calculateSpecificity(b.selectorText);
        if (specA !== specB) return specA - specB;
        // If specificity is equal, the rule appearing later in CSS wins
        // We assume allRules is already in order
        return 0; 
    });

    for (const rule of matchingRules) {
        for (const prop of rule.style) {
            // Rules inside active media queries win over earlier rules
            const matchesMedia = !rule.parentRule || rule.parentRule.type !== CSSRule.MEDIA_RULE || window.matchMedia(rule.parentRule.media.mediaText).matches;
            if (matchesMedia) {
                map[prop] = rule;
            }
        }
...
function isRuleConditional(rule) {
    ...
    if (rule.parentRule && rule.parentRule.type === CSSRule.MEDIA_RULE) return true;
    if (rule.selectorText && rule.selectorText.includes(':')) return true;
    return false;
}
...
function calculateSpecificity(selector) {
    ...
    const tagCount = (selector.match(/(^|[\s>~+])[a-zA-Z]+/g) || []).length;
    
    s += idCount * 100;
    s += (classCount + attrCount + pseudoCount) * 10;
    s += tagCount;
    return s;
}
...
async function identifyGhostClasses() {
    ...
    for (const el of allElements) {
        ...
        for (const cls of el.classList) {
            ...
            if (!state.classIndex.has(cls) && !cls.includes(':')) {
