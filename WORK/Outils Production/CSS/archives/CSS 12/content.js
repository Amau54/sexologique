    ...
    panel.innerHTML = `
        <div class="deepclean-header" id="deepclean-header">
            <div class="deepclean-header-info">
                <h1>DeepClean V7.2 - Smart Assistant</h1>
...
function getDeclaredPropsForClass(allRules, className, element) {
    ...
    for (const r of matchingRules) {
        try {
            ...
            // We want to show properties even for currently inactive variants (e.g. md: if we are on mobile)
            // to provide a complete overview of the class's potential impact.
            
            const isVariantRule = r.selectorText.includes(`.${twEscapedClassName}:`) || 
                                r.selectorText.includes(`.${escapedClassName}:`);

            if (element.matches(r.selectorText) || isVariantRule) {
