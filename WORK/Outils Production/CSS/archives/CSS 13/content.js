...
const state = { 
    ...
    isPicking: false,
    panelPos: { x: 20, y: 20 },
    elementClassMap: new Map() // Tracks all classes (active + disabled) for each element
};
...
async function startDeepClean(targetElement) {
    ...
    if (!state.elementClassMap.has(targetElement)) {
...
async function renderApp() {
    ...
    const scrollPositions = {
        ...
        scanner: scannerList ? scannerList.scrollTop : 0
    };
    ...
    if (newEditorList) newEditorList.scrollTop = scrollPositions.editor;
    if (newScannerList) newScannerList.scrollTop = scrollPositions.scanner;
}
    ...
    panel.innerHTML = `
        <div class="deepclean-header" id="deepclean-header">
            <div class="deepclean-header-info">
                <h1>DeepClean V5 - Smart Assistant</h1>
...
function renderClassCard(cls) {
    ...
    const isTrash = cls.category === 'trash';
    const trashAttribute = isTrash ? 'data-is-trash="true"' : '';
    ...
    return `
        <li class="deepclean-class-card ${ghostClass} ${disabledClass}" data-class-name="${cls.className}" ${trashAttribute}>
            <div class="class-card-main clickable">
                ...
                ${ICONS.chevron}
                <label class="switch">
                    <input type="checkbox" data-class-toggle="${cls.className}" ${isActive && !isTrash ? 'checked' : ''}>
...
function addPanelEventListeners(analysisResults) {
    ...
    sr.getElementById('global-export-btn').addEventListener('click', () => {
        ...
        // Also remove the injected styles
        const injectedStyles = clonedDoc.querySelector('#deepclean-injected-styles');
        if (injectedStyles) injectedStyles.remove();

        const cleanedHtml = clonedDoc.outerHTML;
    ...
    if (addClassInput) {
        ...
        addClassInput.addEventListener('keydown', e => { 
            if (e.key === 'Enter' && addClassInput.value) { 
                state.targetElement.classList.add(addClassInput.value); 
                recordState();
                renderApp(); 
            }
        });
    ...
    if (autocompleteList) {
        autocompleteList.addEventListener('click', e => { 
            if (e.target.classList.contains('autocomplete-item')) { 
                state.targetElement.classList.add(e.target.textContent); 
                recordState();
                renderApp(); 
            }
        });
...
async function analyzeElementCSS(element) {
    ...
    const allRules = await getAllCSSRules(), results = [];
    ...
    for (const className of classList) {
        ...
        if (keptProps.length === 0 && lostProps.length > 0) category = 'trash';
        else if (lostProps.length === 0 && keptProps.length > 0) category = 'essential';
        
        results.push({ className, category, isGhost: false, properties: { kept: keptProps, lost: lostProps } });
    }
...
function getDeclaredPropsForClass(allRules, className, element) {
    ...
    // Improved Regex to avoid false positives (e.g. .btn matching .btn-primary)
