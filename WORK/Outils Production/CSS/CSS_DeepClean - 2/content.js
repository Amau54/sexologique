    ...
    panel.innerHTML = `
        ...
        <div class="deepclean-header" id="deepclean-header">
            <div class="deepclean-header-info">
                <h1>DeepClean V8.0 - Strict Edition</h1>
            ...
            <div class="deepclean-footer">
                <div class="deepclean-footer-actions">
                    <div class="footer-row">
                        <button id="deepclean-clean-btn">✨ Vérifier Élément</button>
                    </div>
                    <div class="footer-row">
                        <button id="global-export-btn">🌍 Exporter Html</button>
                        <button id="reset-all-btn">🔄 Reset Page</button>
                        <button class="fa-magnifying-glass" id="deep-scan-responsive-btn" title="Lancer l'audit responsive">${ICONS.pick}</button>
                    </div>
                </div>
...
async function analyzeElementCSS(element, preFetchedRules = null) {
    ...
    for (const className of classList) {
        ...
        if (keptProps.length === 0) {
            // STRICT TRASH LOGIC: If no visual impact in current view, it's TRASH
            // Only frameworks/SPA dynamic classes provide artificial protection
            if (isDynamic) {
                category = 'partial';
            } else {
                category = 'trash';
        ...
        results.push({ 
            ...
            isInteractive: (isConditional === 'interactive' || protectedByResponsive === 'interactive'),
            isSaved: isDynamic && keptProps.length === 0, 
...
async function identifyGhostClasses() {
    ...
    for (const el of allElements) {
        ...
        // 1. Primary Source of Truth: element.classList (Browser-parsed)
        const activeClasses = Array.from(el.classList);
        
        // 2. Secondary Source (Optional): Restrictive Regex for injected strings
        // We look for class="..." or class='...' in the outerHTML (limited to attribute content)
        const html = el.outerHTML;
        const injectionRegex = /class=["']([^"']+)["']/g;
        let match;
        const injectedClasses = [];
        while ((match = injectionRegex.exec(html)) !== null) {
            injectedClasses.push(...match[1].split(/\s+/));
        }

        const potentialClasses = new Set([...activeClasses, ...injectedClasses]);

        for (let cls of potentialClasses) {
            cls = cls.trim();
            // Filter noise: empty, forbidden chars, or known CSS classes
            if (!cls || forbiddenChars.test(cls)) continue;

            // Tailwind Opacity Support: if cls is "text-red-500/50", check "text-red-500"
            const baseClass = cls.includes('/') ? cls.split('/')[0] : cls;

            // If not in CSS index and not a variant (Tailwind md:, hover: etc are ignored for noise)
